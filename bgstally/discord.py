...
class Discord:
    ...
    def post_to_discord(self, Discord, activity: Activity):
        ...
        if activity.discord_messageid == '' or activity.discord_messageid == None:
            ...
            if discord_text != '':
                ...
                url = self.prefs.DiscordWebhook.get()
                response = requests.post(url=url, params={'wait': 'true'}, data={'content': discord_text, 'username': self.prefs.DiscordUsername.get()})
                if response.ok:
            ...
            if discord_text != '':
                ...
                url = f"{self.prefs.DiscordWebhook.get()}/messages/{activity.discord_messageid}"
                response = requests.patch(url=url, data={'content': discord_text, 'username': self.prefs.DiscordUsername.get()})
                if not response.ok:
    ...
    def is_webhook_valid(self):
        ...
        return self.prefs.DiscordWebhook.get().startswith('https://discordapp.com/api/webhooks/') \
                or self.prefs.DiscordWebhook.get().startswith('https://discord.com/api/webhooks/') \
                or self.prefs.DiscordWebhook.get().startswith('https://ptb.discord.com/api/webhooks/') \
                or self.prefs.DiscordWebhook.get().startswith('https://canary.discord.com/api/webhooks/')
